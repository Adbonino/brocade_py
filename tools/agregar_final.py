...
def read_until_prompt(channel, prompt="admin>"):
    #switch_fabric_D1:FID128:admin>
    buffer = ""
    timeout = 300
    ...
    while True:
        ...
        if time.time() - start_time > timeout:
            print('Salgo de real_until_prompt por timeout')
            return 1
    return 0
...
shell.send(comando + "\n")
print("COMANDO: " + comando)
cod_salida = read_until_prompt(shell, prompt="switch_fabric_D1:FID128:admin>")
print("fin de comando cfgadd")
print(f"cod de salida de read_until_prompt: {cod_salida}")
...
while True:
    if shell.recv_ready():
        ...
        buffer += output         
        # Verifico si alguna de las cadenas de parada aparece en el buffer
        if "Do you want to save" in buffer:
            shell.send("y\n")
            break  
    else:
        # No hay datos nuevos ahora
        time.sleep(0.1)
...
print(f'COMANDO: cfgenable '{fabric_name} )
time.sleep(0.1)
timeout = 240
buffer = ""
start_time = time.time()
